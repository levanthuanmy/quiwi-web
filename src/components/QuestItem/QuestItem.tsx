...
const QuestItem: FC<{ quest: TQuest, onClick: () => void }> = ({ quest, onClick }) => {
    ...
    <div>
      <Row className={classNames(
                  ...
                  .map((item, idx) => (
                    <div key={idx} className={classNames('', styles.layoutImage)}>
                      <Item
                        ...
                        quantity={item.reward.quantity}                      ></Item>
                    </div>
                  ))}
              </div>
        ...
        <Col sm={2} className={classNames('"', styles.button)}>
          <MyButton disabled={quest.userQuest[0].status === "INPROGRESS" || quest.userQuest[0].isReceivedReward} className={classNames("w-100 text-white ")} onClick={() => doneQuest()} >{quest.userQuest[0].isReceivedReward ? "Đã xong" : "Nhận"}</MyButton>
        </Col>
      </Row>
    </div>
  )
