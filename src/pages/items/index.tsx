...
const ItemPage: NextPage = () => {
  ...
  const [itemsResponse, setItemsResponse] = useState<TPaginationResponse<TItem>>();
  const [itemCategoriesResponse, setItemCategoriesResponse] = useState<TPaginationResponse<TItemCategory>>();
  ...
  const getPagination = (totalPages: number, pageCur: number) => {
    if (pageCur > totalPages || pageCur < 1) return currentListPagination ? [...currentListPagination] : [];
    ...
    if (pageCur !== currentPagination && currentListPagination && currentListPagination.indexOf(pageCur) > maxPaginationList / 2 && currentListPagination.length === maxPaginationList) {
    ...
    } else if (pageCur !== currentPagination && currentListPagination && currentListPagination.indexOf(pageCur) < maxPaginationList / 2 && currentListPagination.length === maxPaginationList) {
  ...
  const getItems = async (idCategory: number, pageIndex: number) => {
    if (itemsResponse && (pageIndex > itemsResponse.totalPages || pageIndex < 1)) return;
    const params = {
      filter: {
        where: {
    ...
    try {
      ...
      if (res.response) {
        ...
        if (pageIndex === 1) {
          ...
          setCurrentListPagination(getPagination(res.response.totalPages, pageIndex));
        }
      }
    } catch (error) {
      alert('Có lỗi nè')
      ...
      <div className="d-flex pt-64px min-vh-100" style={{ backgroundColor: '#00A991' }}>
        ...
        >
          <Container fluid="lg">
            <Row className='mt-2 pt-2 align-items-center'>
              ...
              <Col xs={8} style={{ display: 'flex', justifyContent: 'center' }}>
                {itemCategoriesResponse?.items.map((category, idx) => (
                  <div key={category.id} className={toggleState === category.id ? 'tabs active-tabs' : 'tabs'} onClick={() => toggleTab(category.id)}>{category.name}</div>
                ))}
              </Col>
              <Col xs={2}>
            ...
            <Row className={toggleState === toggleState ? 'content' : 'deactive-content'}>
              ...
              <Col style={{ display: 'flex', justifyContent: 'center' }}>
                {itemsResponse ? (
                  <Pagination>
                    ...
                    <Pagination.Prev onClick={() => getItems(toggleState, currentPagination - 1)} />
                    {
                      currentListPagination?.map((item, idx) => (
                        (item === currentPagination ? (
                          <Pagination.Item active>{item}</Pagination.Item>
                        ) : (
                          <Pagination.Item onClick={() => getItems(toggleState, item)}>{item}</Pagination.Item>
                        ))
                      ))
                    }

                    <Pagination.Next onClick={() => getItems(toggleState, currentPagination + 1)} />
                    <Pagination.Last onClick={() => getItems(toggleState, itemsResponse.totalPages)} />
                  </Pagination>
                ) : (<div></div>)}
              </Col>
            </Row>
